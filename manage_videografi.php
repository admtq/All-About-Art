...
if (isset($_GET['delete'])) {
    ...
    header("Location: manage_videografi.php");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    $id = $_POST['id'];
    $judul = $_POST['nama_videografi'];
    $deskripsi = $_POST['deskripsi_videografi'];
    $db->query("UPDATE post SET name_post='$judul', caption_post='$deskripsi' WHERE id_post=$id");
    header("Location: manage_videografi.php");
}

// Ambil semua data
$result = $db->query("SELECT post.id_post, post.name_post, post.caption_post, post_media.media_file FROM post_media INNER JOIN post ON post_media.id_post=post.id_post WHERE media_file LIKE '%videografi%'");
?>
...
<head>
    ...
    <title>Postingan videografi</title>
</head>
<body>
    <!-- Header -->
    <header>
    ...
    <h2>Kelola Postingan videografi</h2>

    <?php if ($edit_item): ?>
        <form method="POST">
            <input type="hidden" name="id" value="<?= $edit_item['id_post'] ?? '' ?>">
            <input type="text" name="nama_videografi" placeholder="Judul videografi" value="<?= $edit_item['name_post'] ?? '' ?>" required>
            <input type="text" name="deskripsi_videografi" placeholder="Deskripsi" value="<?= $edit_item['caption_post'] ?? '' ?>" required>
        ...
        <tbody>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><video controls>
                            <source src="AllAboutArtv2/dbpost/dbvideografi/<?php echo $row['media_file']; ?>">
