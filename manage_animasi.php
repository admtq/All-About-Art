...
if (isset($_GET['delete'])) {
    ...
    header("Location: manage_animasi.php");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    $id = $_POST['id'];
    $judul = $_POST['nama_animasi'];
    $deskripsi = $_POST['deskripsi_animasi'];
    $db->query("UPDATE post SET name_post='$judul', caption_post='$deskripsi' WHERE id_post=$id");
    header("Location: manage_animasi.php");
}

// Ambil semua data
$result = $db->query("SELECT post.id_post, post.name_post, post.caption_post, post_media.media_file FROM post_media INNER JOIN post ON post_media.id_post=post.id_post WHERE media_file LIKE '%animasi%'");
?>
...
<body>
    ...
    <h2>Kelola Postingan animasi</h2>

    <?php if ($edit_item): ?>
        <form method="POST">
            <input type="hidden" name="id" value="<?= $edit_item['id_post'] ?? '' ?>">
            <input type="text" name="nama_animasi" placeholder="Judul animasi" value="<?= $edit_item['name_post'] ?? '' ?>" required>
            <input type="text" name="deskripsi_animasi" placeholder="Deskripsi" value="<?= $edit_item['caption_post'] ?? '' ?>" required>
        ...
        <tbody>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><video controls>
                            <source src="AllAboutArtv2/dbpost/dbanimasi/<?php echo $row['media_file']; ?>">
