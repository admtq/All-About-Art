...
// Ambil data transaksi untuk user ini
$sql = "
    SELECT 
...
<body>
    ...
    <div class="transaction-details">
        ...
        <a href="detail_transaksi.php" class="view-button">Lihat</a>
    </div> -->
        ...
        <?php while ($row = $result_transaksi->fetch_assoc()): ?>
            <div class="post-card">
                ...
                <?php if (preg_match('/^fotografi_/i', $row['media_file'])): ?>
                    <img src="dbpost/dbfotografi/<?php echo htmlspecialchars($row['media_file']); ?>" alt="Gambar Postingan">
                <?php elseif (preg_match('/^videografi_/i', $row['media_file'])): ?>
                    <video controls class="video">
                        <source src="dbpost/dbvideografi/<?php echo htmlspecialchars($row['media_file']); ?>" type="video/mp4">
                        Your browser does not support the video tag.
                    </video>
                <?php elseif (preg_match('/^vector_/i', $row['media_file'])): ?>
                    <img src="dbpost/dbvector/<?php echo htmlspecialchars($row['media_file']); ?>" alt="Vector File">
                <?php elseif (preg_match('/^animasi_/i', $row['media_file'])): ?>
                    <video controls class="video">
                        <source src="dbpost/dbanimasi/<?php echo htmlspecialchars($row['media_file']); ?>" type="video/mp4">
                        Your browser does not support the video tag.
                    </video>
                <?php endif; ?>
                
                <div class="post-content">
                    <h3><?php echo htmlspecialchars($row['name_post']); ?></h3>
                    <p><?php echo htmlspecialchars($row['caption_post']); ?></p>
                </div>
                <div class="transaction-details">
                    <p><strong>Jenis Transaksi:</strong> <?php echo htmlspecialchars($row['jenis_transaksi']); ?></p>
                    <p><strong>Status:</strong> <?php echo htmlspecialchars($row['status_transaksi']); ?></p>
                    <a href="detail_transaksi.php?id_transaksi=<?php echo $row['id_transaksi']; ?>" class="view-button">Lihat</a>
                </div>
            </div>
        <?php endwhile; ?>
    <?php else: ?>
...
footer {
    ...
    padding: 10px; /* Beri ruang di dalam */
}
...
.post-card img {
    ...
    border-radius: 8px;
        ...
        .post-content {
    flex: 2; /* Elemen konten lebih lebar */
    padding: 10px;
}
        ...
        .transaction-details {
    flex: 1; /* Elemen transaksi lebih kecil */
    text-align: right; /* Pastikan teks di kanan */
    font-size: 14px;
}
